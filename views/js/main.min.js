...
changeSliderLabel(size);function changePizzaSizes(size){var newWidth,randomPizza=document.querySelectorAll(".randomPizzaContainer");switch(size){case "1":newWidth=25;break;case "2":newWidth=33.33;break;case "3":newWidth=50;break;default:console.log("bug in sizeSwitcher")}
for(var i=randomPizza.length;i--;){randomPizza[i].style.width=newWidth+'%'}}
changePizzaSizes(size);window.performance.mark("mark_end_resize");window.performance.measure("measure_pizza_resize","mark_start_resize","mark_end_resize");var timeToResize=window.performance.getEntriesByName("measure_pizza_resize");console.log("Time to resize pizzas: "+timeToResize[timeToResize.length-1].duration+"ms")};window.performance.mark("mark_start_generating");for(var i=2;i<30;i++){var pizzasDiv=document.getElementById("randomPizzas");pizzasDiv.appendChild(pizzaElementGenerator(i))}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");var items=document.getElementsByClassName('mover');var docScroll=document.body.scrollTop/1250;for(var i=items.length;i--;){var phase=Math.sin(docScroll+(i%5));var basicLeft=items[i].basicLeft+100*phase+'px'
items[i].style.transform='translateX('+basicLeft+') translateZ(0)'}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;for(var i=40;i--;){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem)}
updatePositions()})